...
import {ProjectAccessLevels, ProjectMember} from "@/lib/entities/project";
...
const CHANGEABLE_LEVELS = [ProjectAccessLevels.write, ProjectAccessLevels.read] as const;
type ChangeableLevel = typeof CHANGEABLE_LEVELS[number];

export default function ProjectAccessBlock({projectId, authorId}: Props) {
    const t = useTranslations("ProjectPage.settings.access");
    ...
    const [pendingLevel, setPendingLevel] = useState<ChangeableLevel>(ProjectAccessLevels.read);
    ...
    async function confirmChangeLevel(userId: string, level: ChangeableLevel | RevokeLevel) {
        addModal((onClose) => (
            <ConfirmModal
                title={t("confirm-title")}
                description={t(`confirm-description-change-${level}`)}
                labelConfirm={t("confirm-label-confirm")}
                labelCancel={t("confirm-label-cancel")}
                onConfirm={() => {
                    onClose();
                    handleChangeLevel(userId, level);
                }}
                onClose={onClose}
            />
        ));
    ...
    async function handleChangeLevel(userId: string, level: ChangeableLevel | RevokeLevel) {
        setLoadingUserId(userId);
        try {
            if (level === REVOKE_LEVEL) {
                const resp = await api.fetch(`/projects/${projectId}/access/${userId}`, {
                    auth: true,
                    method: "DELETE",
                });
                if (!resp.ok) {
                ...
                const resp = await api.fetch(`/projects/${projectId}/access/${userId}`, {
                    ...
                    body: JSON.stringify({userId, accessType: level}),
                });
                if (!resp.ok) {
                    toast.error(t("error-failed"));
                    return;
    ...
    async function handleAddUser() {
        ...
        try {
            const resp = await api.fetch(`/projects/${projectId}/access/${pendingUser.id}`, {
                auth: true,
                method: "PUT",
                headers: {"Content-Type": "application/json"},
                body: JSON.stringify({userId: pendingUser.id, accessType: pendingLevel}),
            });
            if (!resp.ok) {
                toast.error(t("error-failed"));
                return;
            ...
            setPendingUsers([]);
            setPendingLevel(ProjectAccessLevels.read);
            invalidateMembers();
            toast.success(t("toast-success"));
        } catch {
            toast.error(t("error-failed"));
            ...
            {isMembersLoading || isUsersLoading ? (
                ...
                <>
                    <div className="flex items-end gap-2">
                        ...
                        <Select
                            value={pendingLevel}
                            onValueChange={(v) => setPendingLevel(v as ChangeableLevel)}
                        ...
                        >
                            ...
                            <SelectContent>
                                <SelectGroup>
                                    <SelectItem value={ProjectAccessLevels.write}>
                                        {t("level-write")}
                                    </SelectItem>
                                    <SelectItem value={ProjectAccessLevels.read}>
                                        {t("level-read")}
                                    </SelectItem>
                                </SelectGroup>
                            </SelectContent>
                    ...
                    <div className="flex flex-col gap-1">
                        {members?.map(member => {
                            ...
                            const isOwner = member.userId === authorId;
                                ...
                                >
                                    ...
                                    {isOwner ? (
                                        ...
                                        <Select
                                            ...
                                            onValueChange={(v) =>
                                                confirmChangeLevel(member.userId, v as ChangeableLevel | RevokeLevel)
                                            }
